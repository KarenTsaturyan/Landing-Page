const d = document;
let i = 0;
let wallPapers = ["bg1.jpg", "bg2.jpg", "bg3.jpg"];
...
let stories = [text1, text2, text3];
function toRight(){
  //let img = d.getElementById('slx');
  i++;
  if(i >= wallPapers.length){
    i = 0
  }
d.getElementById('myImg').src = wallPapers[i];
d.getElementById('text').innerHTML = stories[i];
}
function toLeft(){
  //let img = d.getElementById('slx');
  i--;
  if(i < 0){
    i = wallPapers.length - 1;
  }
d.getElementById('myImg').src = wallPapers[i];
d.getElementById('text').innerHTML = stories[i];
}
